...
class AtariPreprocessing(object):
    ...
    def reset(self):
        ...
        if self.obs_type == "classic":
            ...
            for i in range(1, P.stack_frames):
                self.screen_buffer[i].fill(0)
    ...
    def step(self, action):
        ...
        for time_step in range(self.frame_skip):
            ...
            elif time_step >= self.frame_skip - P.stack_frames:
                if self.obs_type == "classic":
                    t = time_step - (self.frame_skip - P.stack_frames)
    ...
    def _pool_and_resize(self):
        ...
        transformed_image = [resize(i) for i in self.screen_buffer]

        int_image = np.asarray(transformed_image, dtype=np.uint8)
        # Logger.log(f"{self.screen_buffer[0].shape} {len(self.screen_buffer)} {int_image.shape}")
